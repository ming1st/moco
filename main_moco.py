...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # # ming 
    # backbone_model = timm.create_model(args.arch, num_classes=int(args.moco_dim))
    # model = moco.builder.MoCo(
    #     backbone_model,
    #     args.moco_dim, args.moco_k, args.moco_m, args.moco_t, args.mlp)
    # print(model)
...
def accuracy(output, target, topk=(1,)):
    ...
    with torch.no_grad():
        ...
        res = []
        for k in topk:
            correct_k = correct[:k].reshape(-1).float().sum(0, keepdim=True)        # ming : issue #95
